...
export default function Compatibility() {
  ...
  useEffect(() => {
    ...
    postWithoutImage(tta_obj.api_url + "tta/v1/acf_fields", formData).then(
      (res) => {
  ...
  useEffect(() => {
    ...
    if (hasACFFields) {
      ...
      formData.append("method", "get");
      postWithoutImage(
        tta_obj.api_url + "tta/v1/compatible_data",
        formData,
      ).then((res) => {
        console.log(res.data);
              ...
              <Row className="mt-4">
                <Col xs={12} sm={6} lg={4}>
                  <Form.Label htmlFor="tta__settings_exclude_wp_tags">
                    {__("Add ACF Fields To Posts", "text-to-audio")}{" "}
                          ...
                          <OverlayTrigger
                            ...
                            overlay={
                              <Tooltip id={`tooltip-${placement}`}>
                                {__(
                                  "Adding more than 1 ACF field is a pro feature",
                                  "text-to-audio",
                                )}
                              </Tooltip>
                            }
