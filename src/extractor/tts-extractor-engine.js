...
(function (global) {
     ...
     */
    /**
     * PR-C (C5b): Resolve the stored selector with a most-specific-wins
     * fallback chain. We look up each slot in order and fall through on
     * miss, so a multilingual site with a selector only for English
     * still falls back to the global selector for other languages.
     *
     * Order:
     *   1. opts.savedSelector             — explicit override
     *   2. per_post_type_per_language     — post-type + language
     *   3. per_post_type                  — post-type (any language)
     *   4. per_language                   — language (any post-type)
     *   5. global                         — catch-all
     *
