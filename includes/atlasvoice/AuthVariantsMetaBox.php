...
/**
 * AtlasVoice Auth-Variant Meta Box row (TTS-238 C6b, v5 §14.1 refactor).
 *
 ...
 *
 * When the detector (AuthVariants::get_verdict) reports `differs`, we
 * surface an inline hint explaining that we detected extra/missing
 ...
 *
 * Isolation note (P1/P4): namespace TTA\AtlasVoice — all cross-refs go
 * through \TTA\AtlasVoice\AuthVariants; nothing here reaches back into
 * the legacy TTA\ namespace.
 */
class AuthVariantsMetaBox {
	 ...
	 * AuthVariants and escapes everything it prints.
	 */
	public static function render() {
		if ( ! class_exists( '\\TTA\\AtlasVoice\\AuthVariants' ) ) { return; }
		...
		$desc    = \TTA\AtlasVoice\AuthVariants::describe( $post_id );
		...
		foreach ( $samples as $s ) {
			...
			if ( $b === \TTA\AtlasVoice\AuthVariants::VARIANT_LOGGED_OUT ) { $count_lo++; }
			elseif ( $b === \TTA\AtlasVoice\AuthVariants::VARIANT_LOGGED_IN ) { $count_li++; }
		}
		...
		<div class="tta_auth_variant_row" style="margin-top:16px;padding-top:12px;border-top:1px solid #eee;">
			...
			$options = array(
				...
				\TTA\AtlasVoice\AuthVariants::VARIANT_LOGGED_OUT  => __( 'Logged-out readers', 'text-to-audio' ),
				\TTA\AtlasVoice\AuthVariants::VARIANT_LOGGED_IN   => __( 'Logged-in readers', 'text-to-audio' ),
				\TTA\AtlasVoice\AuthVariants::VARIANT_BOTH        => __( 'Both (render two MP3s)', 'text-to-audio' ),
			);
	...
	public static function save( $post_id ) {
		...
		\TTA\AtlasVoice\AuthVariants::set_variant( $post_id, $val );
	}
}
