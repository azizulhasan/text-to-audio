...
//         update_option('tts_setup_notice_next_show_time', 12);
	...
	public function tta_feedback_notice() {
		...
		if ( $show_notice ) {
			$has_notice = true;
			?>
            <div class="tta-notice notice notice-info is-dismissible" style="line-height:1.5;" data-which="feedback" data-nonce="<?php echo esc_attr( $nonce ); ?>">
			<p><?php
					printf(
					/* translators: 1: plugin name,2: Slightly Smiling Face (Emoji), 3: line break 'br' tag */
					...
					?></p>
                <p>
                    <a class="button button-primary" data-response="given" href="#" target="_blank"><?php esc_html_e( 'Give Feedback Now', \TEXT_TO_AUDIO_TEXT_DOMAIN ); ?></a>
                    <a class="button button-secondary" data-response="later" href="#"><?php esc_html_e( 'Remind Me Later', \TEXT_TO_AUDIO_TEXT_DOMAIN ); ?></a>
                    <a class="button button-secondary" data-response="done" href="#" target="_blank"><?php esc_html_e( 'Already Done!', \TEXT_TO_AUDIO_TEXT_DOMAIN ); ?></a>
                    <a class="button button-secondary" data-response="never" href="#"><?php esc_html_e( 'Never Ask Again', \TEXT_TO_AUDIO_TEXT_DOMAIN ); ?></a>
                </p>
                    ...
                    (function($){
                        ...
                        $(document)
                            .on('click', '.tta-notice a.button', function (e) {
                                ...
                                console.log(self)
                                self.closest(".tta-notice").slideUp( 200, 'linear' );
                                wp.ajax.post( 'tta_save_feedback_notice', { _ajax_nonce: '<?php echo esc_attr( $nonce ); ?>', notice: notice } );
                            })
                            .on('click', '.tta-notice .notice-dismiss', function (e) {
                                e.preventDefault();
                                // noinspection ES6ConvertVarToLetConst
	...
	public function tta_hide_notice() {
		...
		$notices = [  'compitable', 'rating',  'translate', 'promotion_close', 'features', 'feedback', 'setup', 'analytics'  ];
