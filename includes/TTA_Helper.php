...
class TTA_Helper {
	...
	public static function tts_site_language( $plugin_all_settings, $post = '', $btn_no = '' ) {
		$cache_key = '';
		if ( $btn_no && $post ) {
			$cache_key            = 'tts_site_language_post_id_' . $post->ID . '_player_id_' . $btn_no;
	...
	public static function tts_get_voice( $plugin_all_settings, $post = '', $btn_no = '' ) {

		$cache_key = '';
		if ( $btn_no && $post ) {
			$cache_key    = 'tts_get_voice_post_id_' . $post->ID . '_player_id_' . $btn_no;
			$cached_voice = get_transient( $cache_key );
			if ( $cached_voice ) {
				return $cached_voice;
			}
		}
	...
	public static function get_mp3_file_urls_old( $post = '' ) { // TODO: when google cloud TTS is applied. the mp3 file path will be different.
		...
		foreach ( $mp3_file_urls as $language_code => $url ) {
			...
			if ( ! $file_headers || strpos( $file_headers[0], 'Not Found' ) !== false ) {
				...
				$final_mp3_file_urls[ $language_code ] = $url;
			}
		}

		if ( $should_update_urls || empty( $final_mp3_file_urls ) ) {
			update_post_meta( $post->ID, 'tts_mp3_file_urls', $final_mp3_file_urls );
		}
		...
		return \apply_filters( 'tts_mp3_file_urls', $final_mp3_file_urls, $post );
	}
	...
	public static function get_mp3_file_urls( $file_url_key, $post = '', $btn_no = '' ) {
		...
		$cache_key = '';
		if ( $btn_no && $post ) {
			$cache_key   = 'tts_mp3_file_urls_post_id_' . $post->ID . '_player_id_' . $btn_no;
			$cached_urls = get_transient( $cache_key );
			if ( $cached_urls ) {
				$should_return_from_cache = true;
				foreach ( $cached_urls as $language_code => $url ) {
					if ( self::is_file_url_not_exists_and_is_file_empty( $url ) ) {
						$should_return_from_cache = false;
						break;
					}
				}
				if ( $should_return_from_cache ) {
					return $cached_urls;
				}
			}
		}
		...
		if ( $cache_key ) {
			error_log( print_r( [
				'$final_mp3_file_urls' => $final_mp3_file_urls,
				$cache_key
			], 1 ) );
			set_transient( $cache_key, $final_mp3_file_urls );
		}
