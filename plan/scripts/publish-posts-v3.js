...
async function main() {
  ...
  // Create a Code Snippet with 'data' as object (not string)
...
return $results;`;
  ...
  const create = await makeRequest({ jsonrpc: '2.0', id: 10, method: 'tools/call', params: { name: 'awfah-rest-api-run-api-function', arguments: {
    method: 'POST',
    route: '/code-snippets/v1/snippets',
    data: {
      name: 'Publish blog posts Mar 10',
      code: phpCode,
      scope: 'global',
      active: false
    }
  ...
  console.log('Create result:', createText.substring(0, 500));
  ...
  try {
    ...
    console.log('Code saved:', data.code ? 'yes (' + data.code.length + ' chars)' : 'NO - empty');
  } catch(e) {}
  ...
  if (snippetId) {
    ...
    console.log('Activate result:', activateText.substring(0, 500));
    ...
    for (const id of [4173, 4175, 4176, 4177]) {
      ...
      try {
        ...
        console.log(`  Post ${id}: status=${p.status} slug=${p.slug} date=${p.date}`);
      } catch(e) {
