...
async function main() {
  // Fix comparison post slug - use different keyword target to avoid cannibalizing existing /best-text-to-speech-wordpress-plugin/
  const result = await mcpCall(1, 'awfah-rest-api-run-api-function', {
    route: '/wp/v2/posts/4160',
    method: 'POST',
    data: { slug: 'wordpress-text-to-speech-plugins-compared' }
  });

  const text = result.result?.content?.[0]?.text;
  try {
    const post = JSON.parse(text);
    console.log('✅ Slug updated:', post.link);
  } catch(e) {
