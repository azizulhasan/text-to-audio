...
| PR-C D13 (custom-field readers: ACF, MetaBox + stubs) | 🔄 Partial | AcfReader + MetaBoxReader done; Pods/JetEngine/Toolset/CarbonFields pending |
| PR-C D14 + remaining D1–D8 | ⏳ To do | see §13 |
...
| **D13** | **Custom-field reader classes** | `TTA\AtlasVoice\Readers\{ACF,MetaBox,Pods,JetEngine,Toolset,CarbonFields}Reader`. Copy-with-rename from `TTA_Pro_Helper::acf_plugin_content`. F14 substring-dedup guard on append. Compatibility-tab drag-sort UI. | Free (ACF only) + Pro (full) |
