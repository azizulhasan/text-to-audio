# Churn-Reason Analysis â€” July 2026

> Source: `plan/data/2026-07/tracker-2026-07-11/uninstall-reasons.csv` â€” **14,851** deactivation-feedback
...
| **debugging** | 12.8% | 13.3% | **temporary deactivation, not churn** â€” exclude |
...
Two structural facts: (1) **~2/3 of leavers give no usable signal** and ~13% are just debugging â€”
so real, explained churn is the ~1,150 commented rows; (2) the distribution is **stable across
years** â€” churn causes are chronic, not a recent regression.
...
| **Language / accent / pronunciation** | 108 | "not vietnamese voice" Â· TTS providers blocked in Russia Â· multilingual sites stuck with one language |
| Switched / competitor / DIY | 76 | VoxAI Â· "I develop my own plugin" Â· "proper one" |
| Button/player placement | 68 | Can't put the player where they want / player not visible |
| **Mobile** | 46 | "Not working on mobile" repeatedly, Octâ€“Nov 2025 spike |
| Price / cost | 38 | Mostly free-vs-pro friction, see Â§3 |
...
1. **Mobile reliability is the #1 concrete reliability complaint.** A visible run of
   "not working on mobile / phone" comments (Oct 2025 â†’). Web Speech API behavior on
...
2. **The default-voice experience loses users before Pro can win them.** Pattern:
   free browser voice sounds robotic â†’ user can't *hear* the premium voices without paying/signing
   up â†’ uninstall. Direct quote: *"no option to sample the alleged fantastic AI voices without
...
4. **Multilingual is a churn cause, not just a roadmap item.** Missing languages (Vietnameseâ€¦),
...
5. **Two reputation landmines showed up:**
   - A **Patchstack vulnerability listing** was cited as an uninstall reason (Sep 2025) â€” make sure
   ...
   - The **free-version truncation upsell** reads as a dark pattern to users: *"It doesn't read all
     the text, then I see an advertisement saying the paid version fixes itâ€¦ that is crazy."*
...
title"* despite body content selected â€” a current extraction bug (TTS-238 family) worth a ticket.

## 4. Ranked actions (impact Ã· effort)

1. **Mobile QA sweep** of the player (iOS Safari, Android Chrome; Web Speech quirks) â†’ fixes the
   top reliability cluster.
2. **Voice preview without signup** (sample premium voices in-player or on settings) â†’ converts
...
3. **File the title-only extraction bug** from the 2026-07-11 comment; reproduce against current
   `develop`.
4. **Patchstack status check + changelog note** â†’ removes a standing uninstall trigger.
5. **Pricing review of the ElevenLabs BYO-key gate** â†’ founder decision; data says it churns
   technically-savvy users who already pay ElevenLabs.
6. **Soften the truncation-upsell copy** â†’ cheap goodwill fix.
