# Install â†’ Uninstall Lifetime Analysis â€” July 2026

> Source: `plan/data/2026-07/tracker-2026-07-11/` â€” `tracking.csv` (5,614 consented installs, Sep 2023 â†’
> Jul 2026) joined to `uninstall-reasons.csv` (14,851 rows) on **email + plugin**, earliest install
> to first subsequent uninstall. 3,175 unique install identities â†’ **1,399 matched churn pairs**.
...
|---|---|
| **Same day (<24h)** | **63.7%** |
| â†³ of those, **within 1 hour** | **80.9%** (721 users = **51.5% of ALL matched churn**) |
| 1â€“2 days | 8.6% |
| 3â€“7 days | 9.1% |
| 8â€“30 days | 5.8% |
| 31â€“90 days | 4.6% |
...
This is not a retention problem â€” it's a **first-run problem**: install â†’ try â†’ fail to reach
value â†’ gone before lunch.
...
| debugging | 10.4% | **18.8%** |
| looking-for-other | â€” | **4.6%** |

Early leavers say *"I couldn't understand it / it didn't work"* â€” a setup/first-run failure.
...
The AtlasVoice-era same-day churn is ~6.5 points lower â€” consistent with the onboarding wizard
...
1. **The first 60 minutes IS the product.** The single highest-leverage retention work is
   ...
   under a minute â€” ideally an auto-demo on the settings page ("here's your latest post, press
   play") rather than configuration first. Measure it: emit a `first_play` onboarding event and
   track installâ†’first-play conversion.
2. **Triage the first-run failure modes** from the churn-reason analysis (player not visible on
   the page, mobile not working, robotic default voice with no preview) â€” these are exactly what a
...
4. **Don't over-invest in win-back/late-retention** â€” only ~12% of churn happens after day 30.
   The money is all at the front door. (Consistent with: churned users must not be emailed anyway.)

## 6. Method + caveats
Join key = lowercased `admin_email` + `plugin`; earliest install per key; first uninstall â‰¥ install
date; negative-lifetime pairs (uninstall records predating the tracked install, e.g. Freemius-era
installs) excluded. Tracker-era installs only (id â‰¥ 26635). Non-TTS products have too few pairs
(â‰¤2 each) to read. "Text To Speech TTS" vs "AtlasVoice" is the same product across the rebrand.
