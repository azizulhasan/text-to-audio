...
| I3 | Audit for any other inline `<script>` / `<style>` Plugin Check flags | repo-wide | **DONE.** Repo-wide grep found 6 inline blocks; all converted to enqueued assets: CORS detector (`enqueue_cors_detector` + `tta-cors-detector.js` + localize); admin-bar toggle CSS+JS (`enqueue_admin_bar_assets` + `tta-admin-bar.css/js` + localize); deactivation-rescue JS (`tta-deactivation-rescue.js`, HTML keeps allowed `style=""` attrs); posts-list column CSS (`tta-posts-list.css`); browser-support feature-detect + translation-download handler + the JS-injected rotation `<style>` → folded into `tta-admin-notice.js`/`.css` with strings localized via `ttaNoticeData`. **Allowed/excluded:** JSON-LD `<script type="application/ld+json">` (structured-data exception), the `wp_kses`-style sanitization regex in `TTA_Helper`, and `libs/AtlasAiDev/*` (telemetry SDK, out of scope). Verified live: zero plugin inline `<style>`/`<script>` on frontend + posts-list admin page; player + column render; no console errors. | P1 | ✅ |
...
| L2 | (Lookahead) Audit other bundled vendor libs for staleness before SVN push | `admin/js/vendor/*` | **DONE.** Only two shipped vendor libs: `chart.umd.min.js` (4.5.1 — already current from L1) and `countries-and-timezones.min.js` — bumped **3.7.2 → 3.9.0** (latest stable; UMD build, same `getTimezone`/`getCountry` API the analytics use; enqueue version string updated). | ✅ |
...
| F2 | (Defensive) Audit for any other direct `file_put_contents`, `fopen`, `fwrite`, `mkdir`, `unlink` in Free | repo-wide | **DONE — no changes needed.** Grep results: `TTA_Translation_Downloader` already on `WP_Filesystem` (F1); `TTA_Error_Handler` (`fopen`/`fwrite`) and `tta_debug()` (`file_put_contents` to `wp_upload_dir()`) are **deliberate, documented, phpcs-ignored** — WP_Filesystem can't be relied on in a shutdown/error-handler or fire-and-forget debug logger (it may prompt for FTP creds); both write to compliant locations. Other hits are in `plan/` + `vendor/autoload.php`, which are **excluded from the release ZIP**. | ✅ |
...
- **I3** ✅ — all 6 inline `<script>`/`<style>` surfaces converted to enqueued assets (see §3.3). **F2** ✅ — no changes (justified/out-of-scope, see §3.6). **L2** ✅ — `countries-and-timezones` bumped 3.7.2→3.9.0 (see §3.5).
- **Outstanding for 2.2.2 (in priority order)**:
  - B1-B6 → V1-V8 → F1-F7 (version bump, changelog/upgrade-notice, build, **run Plugin Check**, verify, SVN, HelpScout reply) — the release chain is all that remains.
...
- **2026-05-31 (later)** — I3 + F2 + L2 completed on `feature/TTS-247` (Free). I3: converted all 6 inline blocks (CORS detector, admin-bar CSS/JS, deactivation-rescue JS, posts-list CSS, browser-support + translation-download notice JS, rotation-keyframe style) to enqueued files + localized data; verified zero plugin inline `<script>`/`<style>` on frontend + admin. F2: no changes (error-handler/debug-logger direct writes are justified+phpcs-ignored; rest is out-of-ZIP). L2: countries-and-timezones 3.7.2→3.9.0. **All cited items (T1, T2, A1, I1, I2) + defensive audits (I3, F2, L2) done — only the release chain (version bump/changelog/Plugin-Check/build/SVN/reply) remains.**
