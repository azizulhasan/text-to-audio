...
var TTA = {
  ...
  higlightText: function higlightText(tempText) {
    var tempText2 = tempText;
    if (!tempText) {
      return;
    }
    var htmlArr = _toConsumableArray(document.querySelector('.tts_content_wrapper_' + window.buttonId).children);
    htmlArr.map(function (section, index) {
      var currentText = section.innerText;
      console.log(currentText);
      tempText = tempText2;
      var reg = new RegExp(tempText, 'gi');
  ...
  speak: function speak(speech) {
    var content = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : TTA.content;
  ...
  pause: function pause(speech) {
    ...
    if (!TTA.browser.isAndroid()) {
      speech.pause();
      TTA.shouldCancelTimer = setInterval(function () {
        speech.cancel();
        TTA.isCanceled = true;
      }, 7000);
    } else {
    ...
    var currentIndex = TTA.splittedSentances.indexOf(TTA.utterence.target.text);
  ...
  _prepareSpeakButton: function _prepareSpeakButton(speech) {
    // Button click events
