...
/***/ "./src/extractor/tts-extractor-engine.js":
/*!***********************************************!*\
  !*** ./src/extractor/tts-extractor-engine.js ***!
  \***********************************************/
/***/ (function() {
...
(function (global) {
  ...
  var GENERIC_ARTICLE_SELECTORS = ['article .entry-content', 'article .post-content', 'main article', '.entry-content', '.post-content', 'article'];
  var SCHEMA_ARIA_SELECTORS = ['[itemprop="articleBody"]', 'main[role="main"] article', '[role="main"] article', '[role="main"]'];
  ...
  function extractFromMarkers(buttonId) {
    ...
    var walker = global.document.createTreeWalker(global.document.body, NodeFilter.SHOW_COMMENT, null, false);
    var startNode = null;
    var endNode = null;
    var node;
  ...
  var EXCLUDE_ANCESTORS_SELECTOR = 'nav,header,footer,aside,' + '[role="navigation"],[role="banner"],[role="contentinfo"],' + '.menu,.sidebar,.widget,.comments-area,' + '.social-share,.related-posts,.yarpp-related';
  ...
  function matchesBuilderKnown(el) {
    ...
    for (var i = 0; i < BUILDER_CLASS_HINTS.length; i++) {
      if (el.className.indexOf(BUILDER_CLASS_HINTS[i]) !== -1) {
        return true;
      }
    }
    return false;
  ...
  function scoreCandidate(el) {
    ...
    try {
      if (el.closest && el.closest(EXCLUDE_ANCESTORS_SELECTOR)) {
        return 0;
      }
    ...
    if (matchesBuilderKnown(el)) {
      s *= 2.0;
    } // Elementor/Divi/etc.
    if (el.matches && el.matches('[itemprop="articleBody"]')) {
      s *= 1.5;
    }
    if (/(^|\s)entry-content(\s|$)/.test(el.className || '')) {
      s *= 1.5;
    }
    return s;
  }
  ...
  function resolveRuleEntry(opts) {
    ...
    if (opts.postType && lang) {
      ...
      if (langMap && langMap[lang]) {
        entry = normaliseRuleEntry(langMap[lang]);
        if (entry) {
          return entry;
        }
      }
    ...
    if (opts.postType && perType[opts.postType]) {
      entry = normaliseRuleEntry(perType[opts.postType]);
      if (entry) {
        return entry;
      }
    }
    ...
    if (lang) {
      ...
      if (perLang[lang]) {
        entry = normaliseRuleEntry(perLang[lang]);
        if (entry) {
          return entry;
        }
      }
  ...
  function resolveContent(opts) {
    ...
    var heuristicTiers = [{
      name: 'schema',
      selectors: SCHEMA_ARIA_SELECTORS
    }, {
      name: 'builder',
      selectors: allBuilderSelectorStrings()
    }, {
      name: 'article',
      selectors: GENERIC_ARTICLE_SELECTORS
    }];
    for (var i = 0; i < heuristicTiers.length; i++) {
      var best = null;
      try {
        best = pickBestCandidate(heuristicTiers[i].selectors);
      } catch (_) {
        best = null;
      }
      if (best && best.node) {
  ...
  function waitForResolution(opts, callback, timeoutMs) {
    ...
    observer = new global.MutationObserver(function () {
      ...
      if (hint.selector) {
        try {
          if (!global.document.querySelector(hint.selector)) {
            return;
          }
        } catch (_) {
          return;
...
/******/ 	__webpack_modules__["./src/extractor/tts-extractor-engine.js"]();
/******/ 	
/******/ })()
;