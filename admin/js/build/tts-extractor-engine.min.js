...
(function (global) {
  ...
  function resolveContent(opts) {
    ...
    var tiers = [{
      name: 'markers',
      run: function run() {
        return extractFromMarkers(buttonId);
      }
    }, {
      name: 'saved',
      run: function run() {
        return extractFromSelector(savedSelector);
      }
    }, {
      name: 'wrapper',
      run: function run() {
        return extractFromLegacyWrapper(buttonId);
      }
    }, {
      name: 'schema',
      run: function run() {
        return extractFromSelectorList(SCHEMA_ARIA_SELECTORS);
      }
    }, {
      name: 'builder',
      run: function run() {
        return extractFromBuilders();
      }
    }, {
      name: 'article',
      run: function run() {
        return extractFromSelectorList(GENERIC_ARTICLE_SELECTORS);
      }
    }];
